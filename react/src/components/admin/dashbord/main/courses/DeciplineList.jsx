...
/***** React query */
  ...
  return (
    ...
    <table className="discipline_table_admin">
    <thead className='disciplineheader'>
      <tr className='tr_admin'>
        <th className='th_admin' colSpan="">
          Disciplines
        </th>
      </tr>
    </thead>
        ...
        {disciplinesData.map((discipline) => (
          <React.Fragment key={discipline.id}>
            <tr onClick={() => toggleClasses(discipline.id)}
              className={`discipline_row_admin ${expandedDisciplines.includes(discipline.titre) ? 'expanded' : ''} tr_admin`}
            >
              <td className='td_admin'>{discipline.titre}</td>
            ...
            {expandedDisciplines.includes(discipline.id) && (
              <tr className='decipline_classe_tr_admin tr_admin'>
                <td colSpan="2 " className='decipline_classe_tr_admin td_admin' >
                  <ul className='decipline_classe_admin'>
                    {discipline.classes.map((classItem) => (
                    <li className='decipline_classe_item_admin ' key={classItem.id}>
